...
- **Both builds declare a static ruleset** (`rules/blocklist-rules.json`,
  generated by `scripts/build-dnr-ruleset.mjs`). It uses the `requestDomains`
  condition, which is Chrome 101+ and Firefox 113+ — that is why
  `minimum_chrome_version` is 101. Firefox needed no change: it shipped static
  rulesets and `requestDomains` together in 113, already the declared floor.
- **Content scripts run in all frames** (`all_frames` + `match_about_blank`).
  Frames under 120px on either axis bail before reading settings; page-level
  scans and page-level UI stay in the top frame. See the note at the top of
  `content.js`.
