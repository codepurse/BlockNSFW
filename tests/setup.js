...
const PUBLIC_SUFFIX_TEXT = fs.readFileSync(
  path.join(__dirname, '..', 'data', 'public-suffixes.txt'), 'utf8');
...
const SHARED_PUBLIC_SUFFIX_PATH = path.join(__dirname, '..', 'shared', 'public-suffix.js');
const SHARED_DOMAIN_POLICY_PATH = path.join(__dirname, '..', 'shared', 'domain-policy.js');
...
function loadBackgroundContext(sourcePath = SOURCE_PATH, customize) {
  ...
  const sharedPublicSuffixSource = fs.readFileSync(SHARED_PUBLIC_SUFFIX_PATH, 'utf8');
  const sharedDomainPolicySource = fs.readFileSync(SHARED_DOMAIN_POLICY_PATH, 'utf8');
  ...
  const sandbox = {
    ...
    console,
    // data/public-suffixes.txt is a packaged file the blocklist guard reads on
    // every load, not a remote call. Serve the real one so tests see production
    // semantics: half the old SHARED_CDN_PARENT_DOMAINS list now relies on the
    // PSL covering it, so an empty list would quietly change which parents are
    // skipped.
    fetch: (url) => String(url).endsWith('public-suffixes.txt')
      ? Promise.resolve({ ok: true, text: async () => PUBLIC_SUFFIX_TEXT })
  ...
  for (const [file, label] of [[sharedBrowserKeySource, 'shared/browser-key.js'], [sharedHostnameSource, 'shared/hostname.js'], [sharedHostKeywordsSource, 'shared/host-keywords.js'], [sharedValidateDomainSource, 'shared/validate-domain.js'], [sharedKeywordPatternSource, 'shared/keyword-pattern.js'], [sharedPublicSuffixSource, 'shared/public-suffix.js'], [sharedDomainPolicySource, 'shared/domain-policy.js'], [sharedDnsProvidersSource, 'shared/dns-providers.js'], [sharedAiImageModelsSource, 'shared/ai-image-models.js'], [sharedVitClassifierSource, 'shared/vit-classifier.js']]) {
    try {
